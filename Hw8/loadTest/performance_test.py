"""
HW8 Performance Test — 150 Operations Against MySQL-Backed Shopping Cart API
=============================================================================
Test Protocol:
  - 50x POST /shopping-carts       (create cart)
  - 50x POST /shopping-carts/{id}/items (add items)
  - 50x GET  /shopping-carts/{id}   (retrieve cart)
  - Must complete within 5 minutes
  - Results saved to mysql_test_results.json

Usage:
  python performance_test.py --host http://<ECS_PUBLIC_IP>:8080
"""
...
def run_test(base_url):
    ...
    print(f"\n{'='*60}")
    print(f"  HW8 MySQL Performance Test")
    print(f"  Target: {base_url}")
    print(f"  Time:   {datetime.now(timezone.utc).isoformat()}")
    print(f"{'='*60}\n")

    # ── Phase 1: Create 50 carts ─────────────────────────────
    print("[Phase 1] Creating 50 shopping carts...")
    for i in range(50):
        customer_id = 1000 + i
    ...
    print("[Phase 3] Retrieving 50 carts...")
    for i in range(50):
        if not cart_ids:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="HW8 MySQL Performance Test")
