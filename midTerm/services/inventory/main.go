...
func main() {
	name := getEnv("SERVICE_NAME", "Inventory")
	delayMs, _ := strconv.Atoi(getEnv("BASE_DELAY_MS", "30"))
	...
	// Health check — ALB target group polls this
	...
	// Main request handler — called by Order API
	mux.HandleFunc("/process", func(w http.ResponseWriter, r *http.Request) {
		if !svc.IsHealthy() {
			// 50% chance: hang (timeout), 50% chance: immediate 500
			if rand.Float64() < 0.5 {
